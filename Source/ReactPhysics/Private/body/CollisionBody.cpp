...
* @return The number of colliders associated with this body
*/
uint8 CollisionBody::getNbColliders() const {
    return static_cast<uint8>(mWorld.mCollisionBodyComponents.getColliders(mEntity).size());
}
...
*/
const Collider* CollisionBody::getCollider(uint8 colliderIndex) const {
...
*/
Collider* CollisionBody::getCollider(uint8 colliderIndex) {
...
void CollisionBody::removeCollider(Collider* collider) {
    ...
    RP3D_LOG(mWorld.mConfig.worldName, Logger::Level::Information, Logger::Category::Body,
             "Body " + URealFloatMath::ConvRealToString(mEntity.id) + ": Collider " + URealFloatMath::ConvRealToString(collider->getBroadPhaseId()) + " removed from body",  __FILE__, __LINE__);
...
void CollisionBody::removeAllColliders() {
    ...
    for (uint8 i=0; i < collidersEntities.size(); i++) {
...
void CollisionBody::setIsActive(bool isActive) {
    ...
    RP3D_LOG(mWorld.mConfig.worldName, Logger::Level::Information, Logger::Category::Body,
             "Body " + URealFloatMath::ConvRealToString(mEntity.id) + ": Set isActive=" +
...
AABB CollisionBody::getAABB() const {
    ...
    // For each collider of the body
    for (uint8 i=1; i < colliderEntities.size(); i++) {
...
void CollisionBody::setTransform(const Transform& transform) {
    ...
    RP3D_LOG(mWorld.mConfig.worldName, Logger::Level::Information, Logger::Category::Body,
             "Body " + URealFloatMath::ConvRealToString(mEntity.id) + ": Set transform=" + transform.to_string(),  __FILE__, __LINE__);
}
