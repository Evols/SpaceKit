...
void NarrowPhaseInfoBatch::addContactPoint(uint8 index, const Vector3& contactNormal, decimal penDepth,
...
void NarrowPhaseInfoBatch::resetContactPoints(uint8 index) {

    // Get the memory allocator
    MemoryAllocator& allocator = mOverlappingPairs.getTemporaryAllocator();

    // For each remaining contact point info
    for (uint8 i=0; i < contactPoints[index].size(); i++) {
...
void NarrowPhaseInfoBatch::clear() {
    ...
    for (uint8 i=0; i < overlappingPairIds.size(); i++) {
