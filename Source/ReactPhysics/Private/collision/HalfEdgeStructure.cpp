...
void HalfEdgeStructure::init() {
    ...
    Map<VerticesPair, uint8> mapEdgeToStartVertex(mAllocator);
    Map<VerticesPair, uint8> mapEdgeToIndex(mAllocator);
    Map<uint8, VerticesPair> mapEdgeIndexToKey(mAllocator);
    Map<uint8, VerticesPair> mapFaceIndexToEdgeKey(mAllocator);

    List<VerticesPair> currentFaceEdges(mAllocator, mFaces[0].faceVertices.size());

    // For each face
    for (uint8 f=0; f<mFaces.size(); f++) {
        ...
        for (uint8 v=0; v < face.faceVertices.size(); v++) {
            uint8 v1Index = face.faceVertices[v];
            uint8 v2Index = face.faceVertices[v == (face.faceVertices.size() - 1) ? 0 : v + 1];
            ...
            mapEdgeToStartVertex.add(Pair<VerticesPair, uint8>(pairV1V2, v1Index), true);
            mapEdgeToStartVertex.add(Pair<VerticesPair, uint8>(pairV2V1, v2Index), true);

            mapFaceIndexToEdgeKey.add(Pair<uint8, VerticesPair>(f, pairV1V2), true);

            auto itEdge = edges.find(pairV2V1);
            if (itEdge != edges.end()) {

                const uint8 edgeIndex = mEdges.size();

                itEdge->second.twinEdgeIndex = edgeIndex + 1;
                edge.twinEdgeIndex = edgeIndex;

                mapEdgeIndexToKey.add(Pair<uint8, VerticesPair>(edgeIndex, pairV2V1));
                mapEdgeIndexToKey.add(Pair<uint8, VerticesPair>(edgeIndex + 1, pairV1V2));

                mVertices[v1Index].edgeIndex = edgeIndex + 1;
                mVertices[v2Index].edgeIndex = edgeIndex;

                mapEdgeToIndex.add(Pair<VerticesPair, uint8>(pairV1V2, edgeIndex + 1));
                mapEdgeToIndex.add(Pair<VerticesPair, uint8>(pairV2V1, edgeIndex));
