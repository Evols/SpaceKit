...
//#include <stdio.h>
#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btDiscreteCollisionDetectorInterface.h"
#include "SpaceKitBullet/BulletCollision/BroadphaseCollision/btBroadphaseInterface.h"
#include "SpaceKitBullet/BulletCollision/CollisionDispatch/btCollisionObject.h"
#include "SpaceKitBullet/BulletCollision/CollisionShapes/btConvexShape.h"
#include "SpaceKitBullet/BulletCollision/CollisionShapes/btCapsuleShape.h"
#include "SpaceKitBullet/BulletCollision/CollisionShapes/btTriangleShape.h"
#include "SpaceKitBullet/BulletCollision/CollisionShapes/btConvexPolyhedron.h"

#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btGjkPairDetector.h"
...
#include "SpaceKitBullet/BulletCollision/CollisionDispatch/btManifoldResult.h"

#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btConvexPenetrationDepthSolver.h"
#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btContinuousConvexCollision.h"
#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btSubSimplexConvexCast.h"
#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btGjkConvexCast.h"

#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btVoronoiSimplexSolver.h"
#include "SpaceKitBullet/BulletCollision/CollisionShapes/btSphereShape.h"

#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btMinkowskiPenetrationDepthSolver.h"

#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btGjkEpa2.h"
#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btGjkEpaPenetrationDepthSolver.h"
#include "SpaceKitBullet/BulletCollision/NarrowPhaseCollision/btPolyhedralContactClipping.h"
#include "SpaceKitBullet/BulletCollision/CollisionDispatch/btCollisionObjectWrapper.h"
	...
	const btVector3& dirB, btScalar hlenB)
{
	...
	btScalar denom = 1.0_fl - dirA_dot_dirB * dirA_dot_dirB;
...
//#define DEBUG_CONTACTS 1
		...
		m_debugDrawer->drawLine(startPt, endPt, btVector3(1.0_fl, 0.0_fl, 0.0_fl));
...
void btConvexConvexAlgorithm ::processCollision(const btCollisionObjectWrapper* body0Wrap, const btCollisionObjectWrapper* body1Wrap, const btDispatcherInfo& dispatchInfo, btManifoldResult* resultOut)
{
	...
	if ((min0->getShapeType() == CAPSULE_SHAPE_PROXYTYPE) && (min1->getShapeType() == SPHERE_SHAPE_PROXYTYPE))
	{
		...
		btScalar dist = capsuleCapsuleDistance(normalOnB, pointOnBWorld, capsuleA->getHalfHeight(), capsuleA->getRadius(),
											   0.0_fl, capsuleB->getRadius(), capsuleA->getUpAxis(), 1,
											   body0Wrap->getWorldTransform(), body1Wrap->getWorldTransform(), threshold);
	...
	if ((min0->getShapeType() == SPHERE_SHAPE_PROXYTYPE) && (min1->getShapeType() == CAPSULE_SHAPE_PROXYTYPE))
	{
		...
		btScalar dist = capsuleCapsuleDistance(normalOnB, pointOnBWorld, 0.0_fl, capsuleA->getRadius(),
	...
	if (!dispatchInfo.m_useConvexConservativeDistanceUtil || m_sepDistance.getConservativeSeparatingDistance() <= 0.0_fl)
...

	{
		...
		if (min0->isPolyhedral() && min1->isPolyhedral())
		{
			...
			btScalar min0Margin = min0->getShapeType() == BOX_SHAPE_PROXYTYPE ? 0.0_fl : min0->getMargin();
			btScalar min1Margin = min1->getShapeType() == BOX_SHAPE_PROXYTYPE ? 0.0_fl : min1->getMargin();
			...
			if (polyhedronA->getConvexPolyhedron() && polyhedronB->getConvexPolyhedron())
			{
				...
				else
				{
					...
					//if (l2>SIMD_EPSILON)
					{
						sepNormalWorldSpace = withoutMargin.m_reportedNormalOnWorld;  //gjkPairDetector.getCachedSeparatingAxis()*(1.0_fl/l2);
						//minDist = -1e30_fl;//gjkPairDetector.getCachedSeparatingDistance();
						minDist = withoutMargin.m_reportedDistance;  //gjkPairDetector.getCachedSeparatingDistance()+min0->getMargin()+min1->getMargin();

#ifdef ZERO_MARGIN
						foundSepAxis = true;  //gjkPairDetector.getCachedSeparatingDistance()<0.0_fl;
#else
						foundSepAxis = withoutMargin.m_foundResult && minDist < 0_fl;  //-(min0->getMargin()+min1->getMargin());
#endif
					}
				}
				if (foundSepAxis)
			...
			else
			{
				...
				if (dispatchInfo.m_enableSatConvex && polyhedronA->getConvexPolyhedron() && polyhedronB->getShapeType() == TRIANGLE_SHAPE_PROXYTYPE)
				{
					...
					else
					{
						...
						if (dummy.m_hasContact && dummy.m_depth < 0_fl)
						{
							if (foundSepAxis)
							{
								if (dummy.m_normalOnBInWorld.dot(sepNormalWorldSpace) < 0.99_fl)
								{
									printf("?\n");
								}
							}
							...
							sepNormalWorldSpace.setValue(0_fl, 0_fl, 1_fl);  // = dummy.m_normalOnBInWorld;
					...
					if (l2>SIMD_EPSILON)
					{
						sepNormalWorldSpace = gjkPairDetector.getCachedSeparatingAxis()*(1.0_fl/l2);
