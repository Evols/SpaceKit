...
#include "SpaceKitBullet/btBulletCollisionCommon.h"
#include "SpaceKitBullet/LinearMath/btSerializer.h"  //for btBulletSerializedArrays definition

#ifdef SUPPORT_GIMPACT_SHAPE_IMPORT
#include "SpaceKitBullet/BulletCollision/Gimpact/btGImpactShape.h"
...
bool btCollisionWorldImporter::convertAllObjects(btBulletSerializedArrays* arrays)
{
	...
	for (i = 0; i < arrays->m_collisionObjectDataDouble.size(); i++)
	{
		...
		if (shapePtr && *shapePtr)
		{
			...
			colObjData->m_worldTransform.m_origin.m_floats[3] = 0.0_fl;
	...
	for (i = 0; i < arrays->m_collisionObjectDataFloat.size(); i++)
	{
		...
		{
			btTransform startTransform;
			colObjData->m_worldTransform.m_origin.m_floats[3] = 0.0;
	...
	switch (shapeData->m_shapeType)
	{
		...
		case CAPSULE_SHAPE_PROXYTYPE:
		{
			...
			switch (capData->m_upAxis)
			{
				case 0:
				{
					shape = createCapsuleShapeX(1_fl, 1_fl);
					break;
				}
				case 1:
				{
					shape = createCapsuleShapeY(1_fl, 1_fl);
					break;
				}
				case 2:
				{
					shape = createCapsuleShapeZ(1_fl, 1_fl);
					break;
				}
				default:
				{
