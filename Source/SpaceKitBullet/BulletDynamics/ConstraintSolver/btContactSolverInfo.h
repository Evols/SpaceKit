...
struct btContactSolverInfo : public btContactSolverInfoData
{
	...
	{
		m_tau = btScalar(0.6);
		m_damping = btScalar(1.0_fl);
		m_friction = btScalar(0.3);
		m_timeStep = btScalar(1.0_fl / 60_fl);
		m_restitution = btScalar(0.0_fl);
		m_maxErrorReduction = btScalar(20.0_fl);
		m_numIterations = 10;
		m_erp = btScalar(0.2_fl);
		m_erp2 = btScalar(0.2_fl);
		m_deformable_erp = btScalar(0.06);
		m_deformable_cfm = btScalar(0.01);
		m_deformable_maxErrorReduction = btScalar(0.1_fl);
		m_globalCfm = btScalar(0.0_fl);
		m_frictionERP = btScalar(0.2_fl);  //positional friction 'anchors' are disabled by default
		m_frictionCFM = btScalar(0.0_fl);
		m_sor = btScalar(1.);
		m_splitImpulse = true;
		m_splitImpulsePenetrationThreshold = -.04_fl;
		m_splitImpulseTurnErp = 0.1_fl;
		m_linearSlop = btScalar(0.0_fl);
		...
		m_maxGyroscopicForce = 100_fl;                          ///it is only used for 'explicit' version of gyroscopic force
		...
		m_leastSquaresResidualThreshold = 0.0_fl;
		m_restitutionVelocityThreshold = 0.2_fl;  //if the relative velocity is below this threshold, there is zero restitution
