...
#include "SpaceKitBullet/Bullet3Common/shared/b3Quat.h"

#ifdef __cplusplus

#include "SpaceKitBullet/Bullet3Common/b3Matrix3x3.h"
...
{
	b3Float4 quat2 = (b3Float4)(quat.x * quat.x, quat.y * quat.y, quat.z * quat.z, 0.0_fl);
	b3Mat3x3 out;

	out.m_row[0].x = 1 - 2 * quat2.y - 2 * quat2.z;
	out.m_row[0].y = 2 * quat.x * quat.y - 2 * quat.w * quat.z;
	out.m_row[0].z = 2 * quat.x * quat.z + 2 * quat.w * quat.y;
	out.m_row[0].w = 0.0_fl;

	out.m_row[1].x = 2 * quat.x * quat.y + 2 * quat.w * quat.z;
	out.m_row[1].y = 1 - 2 * quat2.x - 2 * quat2.z;
	out.m_row[1].z = 2 * quat.y * quat.z - 2 * quat.w * quat.x;
	out.m_row[1].w = 0.0_fl;

	out.m_row[2].x = 2 * quat.x * quat.z - 2 * quat.w * quat.y;
	out.m_row[2].y = 2 * quat.y * quat.z + 2 * quat.w * quat.x;
	out.m_row[2].z = 1 - 2 * quat2.x - 2 * quat2.y;
	out.m_row[2].w = 0.0_fl;
...
{
	b3Mat3x3 m;
	m.m_row[0] = (b3Float4)(0.0_fl);
	m.m_row[1] = (b3Float4)(0.0_fl);
	m.m_row[2] = (b3Float4)(0.0_fl);
	return m;
}
...
{
	b3Mat3x3 out;
	out.m_row[0] = (b3Float4)(m.m_row[0].x, m.m_row[1].x, m.m_row[2].x, 0.0_fl);
	out.m_row[1] = (b3Float4)(m.m_row[0].y, m.m_row[1].y, m.m_row[2].y, 0.0_fl);
	out.m_row[2] = (b3Float4)(m.m_row[0].z, m.m_row[1].z, m.m_row[2].z, 0.0_fl);
	return out;
}
